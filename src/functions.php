...
/**
 * Helper function that convert hump string to underline style
 * @param string $humpStr
 * @return string|string[]|null
 */
function hump2underline($humpStr)
{
    //eBay is a special word,because its first letter is not upper case
    $strNotContainEbay = preg_replace('/ebay/i',' ',$humpStr);
    if ($strNotContainEbay === ' ') {
        return 'EBAY';
    }
    //deal normal hump string
    $tmp = preg_replace('/([a-z0-9])([A-Z])/', "$1_$2", $strNotContainEbay);

    //compatible words with all upper case.e.g.VATExempt,USPSRetail
    $tmp = strtoupper(preg_replace('/([A-Z])([A-Z])([a-z])/',"$1_$2$3",$tmp));
    return preg_replace(['/^ /','/([A-Z]) ([A-Z])/','/([A-Z]) $/'],['EBAY_',"$1_EBAY_$2","$1_EBAY"],$tmp);
}
